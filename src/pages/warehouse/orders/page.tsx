...
export default function WarehouseOrders() {
  ...
  const orders = [
    {
      ...
      parts: [
        ...
        { code: 'BRK-3301', name: '브레이크 디스크', quantity: 8, price: 120000 },
    ...
    },
    {
      ...
      orderDate: '2024-01-15',
      parts: [
        { code: 'ELC-9902', name: '헤드라이트 벌브', quantity: 4, price: 45000 },
    ...
    },
    {
      ...
      parts: [
        ...
        { code: 'SUS-2233', name: '서스펜션 스프링', quantity: 8, price: 180000 },
    ...
    },
    {
      ...
      parts: [
        ...
        { code: 'RAD-4455', name: '라디에이터 호스', quantity: 6, price: 35000 }
      ]
    },
    {
      id: 'WH-2024-0115-005',
      ...
      orderDate: '2024-01-15',
      parts: [
        { code: 'WIN-6677', name: '와이퍼 블레이드', quantity: 4, price: 28000 },
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'pickup_ready': return 'bg-purple-100 text-purple-800 border-purple-200';
  ...
  const getStatusText = (status: string) => {
    switch (status) {
      ...
      case 'pickup_ready': return '픽업 대기';
      case 'packaging': return '포장중';
  ...
  const processingOrders = orders.filter(o => o.status === 'processing' || o.status === 'packaging').length;
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <div className="bg-white dark:bg-gray-800 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600 dark:text-gray-400">오늘 총 주문</p>
                <p className="text-2xl font-bold text-gray-900 dark:text-white">{totalOrders}</p>
                <p className="text-xs text-blue-600 dark:text-blue-400 mt-1">접수된 주문</p>
              </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-orange-600 dark:text-orange-400 mt-1">처리 필요</p>
              </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600 dark:text-gray-400">처리중인 주문</p>
                <p className="text-2xl font-bold text-gray-900 dark:text-white">{processingOrders}</p>
                <p className="text-xs text-purple-600 dark:text-purple-400 mt-1">작업 진행중</p>
              </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600 dark:text-gray-400">총 주문 금액</p>
                <p className="text-2xl font-bold text-gray-900 dark:text-white">₩{(totalValue / 1000000).toFixed(1)}M</p>
                <p className="text-xs text-green-600 dark:text-green-400 mt-1">오늘 매출</p>
              </div>
          ...
          <div className="p-4">
            <div className="space-y-4">
              {filteredOrders.map((order) => (
                <div key={order.id} className="p-4 bg-gray-50 dark:bg-gray-700 rounded-lg border border-gray-200 dark:border-gray-600">
                  <div className=" items-start justify-between mb-3">
                    <div className="-1">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                        <div>
                          <p className="text-gray-600 dark:text-gray-400">대리점</p>
                          <p className="font-medium text-gray-900 dark:text-white">{order.dealer}</p>
                        </div>
                        <div>
                          <p className="text-gray-600 dark:text-gray-400">담당자</p>
                          <p className="font-medium text-gray-900 dark:text-white">{order.contact}</p>
                        </div>
                        <div>
                          <p className="text-gray-600 dark:text-gray-400">주문 시간</p>
                          <p className="font-medium text-gray-900 dark:text-white">{order.orderTime}</p>
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center space-x-2">
                      ...
                      <Button variant="secondary" size="sm" onClick={() => handlePrintOrder(order)}>
                        <i className="ri-printer-line"></i>
                      </Button>
                      <Button variant="default" size="sm" onClick={() => handleProcessOrder(order)}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm mb-3">
                    <div>
                      ...
                      <p className="font-medium text-gray-900 dark:text-white">{order.items}개</p>
                    </div>
                    <div>
                      <p className="text-gray-600 dark:text-gray-400">총 금액</p>
                      <p className="font-medium text-gray-900 dark:text-white">₩{order.totalValue.toLocaleString()}</p>
                    </div>
                    <div>
                      <p className="text-gray-600 dark:text-gray-400">예상 픽업</p>
                      <p className="font-medium text-gray-900 dark:text-white">{order.estimatedPickup}</p>
                    </div>
                    <div>
                      <p className="text-gray-600 dark:text-gray-400">배송지</p>
                      <p className="font-medium text-gray-900 dark:text-white text-xs">{order.address}</p>
                    </div>
                  </div>

                  {/* Parts List */}
                  <div className="mt-3 p-3 bg-white dark:bg-gray-800 rounded border">
                    <h4 className="text-sm font-medium text-gray-900 dark:text-white mb-2">주문 부품 목록</h4>
                    <div className="space-y-1">
                      {order.parts.map((part, index) => (
                        <div key={index} className="flex items-center justify-between text-xs">
                          <div className="flex items-center space-x-2">
                            <span className="text-gray-500 dark:text-gray-400">{part.code}</span>
                            <span className="text-gray-900 dark:text-white">{part.name}</span>
                          </div>
                          <div className="flex items-center space-x-2">
                            <span className="text-gray-600 dark:text-gray-400">{part.quantity}개</span>
                            <span className="font-medium text-gray-900 dark:text-white">₩{part.price.toLocaleString()}</span>
                          </div>
                        </div>
                      ))}
                    </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-2xl mx-4">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-gray-900 dark:text-white">수동 주문 등록</h3>
                <button
              ...
              <div className="space-y-4">
                ...
                <div className="border-t pt-4">
                  <h4 className="font-medium text-gray-900 dark:text-white mb-3">주문 부품</h4>
        ...
        {showOrderDetailModal && selectedOrder && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-4xl mx-4 max-h-[90vh] overflow-y-auto">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-gray-900 dark:text-white">주문 상세 정보</h3>
                <button
              ...
              <div className="grid grid-cols-2 gap-6 mb-6">
                <div>
                  <h4 className="font-medium text-gray-900 dark:text-white mb-3">주문 정보</h4>
                  <div className="space-y-2 text-sm">
                    ...
                    <div className=" justify-between">
                      ...
                      <span className="font-medium text-gray-900 dark:text-white">{selectedOrder.orderDate} {selectedOrder.orderTime}</span>
                    </div>
                    <div className=" justify-between">
                ...
                <div>
                  <h4 className="font-medium text-gray-900 dark:text-white mb-3">대리점 정보</h4>
                  <div className="space-y-2 text-sm">
                    <div className=" justify-between">
                      <span className="text-gray-600 dark:text-gray-400">대리점명:</span>
                      <span className="font-medium text-gray-900 dark:text-white">{selectedOrder.dealer}</span>
                    </div>
                    <div className=" justify-between">
                      <span className="text-gray-600 dark:text-gray-400">담당자:</span>
                      <span className="font-medium text-gray-900 dark:text-white">{selectedOrder.contact}</span>
                    </div>
                    <div className=" justify-between">
                      <span className="text-gray-600 dark:text-gray-400">배송지:</span>
                      <span className="font-medium text-gray-900 dark:text-white text-xs">{selectedOrder.address}</span>
                    </div>
                    <div className=" justify-between">
                      <span className="text-gray-600 dark:text-gray-400">예상 픽업:</span>
                      <span className="font-medium text-gray-900 dark:text-white">{selectedOrder.estimatedPickup}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="mb-6">
                <h4 className="font-medium text-gray-900 dark:text-white mb-3">주문 부품 상세</h4>
                <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                      ...
                      <div key={index} className="flex items-center justify-between p-3 bg-white dark:bg-gray-800 rounded border">
                        <div className="-1">
                          <div className="flex items-center space-x-3">
                            <span className="text-sm font-mono text-gray-500 dark:text-gray-400">{part.code}</span>
                            <span className="font-medium text-gray-900 dark:text-white">{part.name}</span>
                          </div>
                        </div>
                        <div className="flex items-center space-x-4 text-sm">
                          <span className="text-gray-600 dark:text-gray-400">수량: {part.quantity}개</span>
                          <span className="font-medium text-gray-900 dark:text-white">₩{part.price.toLocaleString()}</span>
                          <span className="font-semibold text-gray-900 dark:text-white">₩{(part.quantity * part.price).toLocaleString()}</span>
                        </div>
                      </div>
                    ))}
                  </div>
                  <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-600">
                    <div className=" justify-between items-center">
                      <span className="text-lg font-semibold text-gray-900 dark:text-white">총 주문 금액:</span>
                      <span className="text-xl font-bold text-gray-900 dark:text-white">₩{selectedOrder.totalValue.toLocaleString()}</span>
                    </div>
                  </div>
                </div>
              </div>
