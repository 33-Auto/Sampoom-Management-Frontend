...
export default function HRMEvaluation() {
  ...
  const evaluationData = [
    {
      ...
      goals: [
        ...
        { title: '기술 교육 프로그램 운영', status: 'in_progress', progress: 80 }
      ],
      feedback: '뛰어난 기술 리더십과 팀 관리 능력을 보여주었습니다. 신규 프로젝트를 성공적으로 완료하였고, 팀원들의 성장을 위한 교육에도 적극적입니다.',
      nextGoals: ['차세대 아키텍처 설계', '주니어 개발자 멘토링', '기술 블로그 운영'],
      evaluator: '이사 김대표',
      status: 'completed'
    },
    {
      ...
      feedback: '창의적인 마케팅 전략으로 브랜드 가치 향상에 크게 기여했습니다. 다양한 채널을 활용한 통합 마케팅 캠페인이 성공적이었습니다.',
      nextGoals: ['글로벌 마케팅 전략 수립', '인플루언서 마케팅 확대', '데이터 기반 마케팅 도입'],
      evaluator: '이사 박부장',
      status: 'completed'
    },
    {
      ...
      feedback: '목표 매출을 초과 달성하며 뛰어난 영업 성과를 보였습니다. 고객 관계 관리에도 우수한 능력을 발휘했습니다.',
      nextGoals: ['대형 고객사 확보', '영업 프로세스 개선', '후배 영업사원 교육'],
      evaluator: '이사 최부장',
      status: 'completed'
    },
    {
      ...
      goals: [
        { title: '신입사원 온보딩 프로그램 개선', status: 'completed', progress: 100 },
        { title: '직원 만족도 조사 실시', status: 'completed', progress: 100 },
        { title: '교육 프로그램 3개 신설', status: 'in_progress', progress: 70 }
      ],
      feedback: '체계적인 인사 관리와 직원 복지 향상에 크게 기여했습니다. 소통 능력이 뛰어나 직원들의 신뢰를 받고 있습니다.',
      nextGoals: ['성과 관리 시스템 도입', '리더십 교육 프로그램 개발', '조직문화 개선 활동'],
      evaluator: '이사 김대표',
      status: 'in_progress'
    },
    {
      ...
      feedback: '정확한 재무 분석과 비용 관리로 회사 경영에 도움을 주고 있습니다. 시스템 구축 역량을 더 발전시킬 필요가 있습니다.',
      nextGoals: ['ERP 시스템 고도화', '투자 분석 역량 강화', '재무 전략 수립 참여'],
      evaluator: '이사 박부장',
      status: 'in_progress'
    }
  ];
  ...
  const highPerformers = filteredEvaluations.filter(e => e.overallScore >= 4.5).length;
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-white rounded-lg p-6 border border-gray-200">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600">평균 평가점수</p>
                <p className="text-2xl font-bold text-gray-900">{avgScore.toFixed(1)}</p>
                <p className="text-xs text-blue-600 mt-1">5점 만점</p>
              </div>
          ...
          <div className="bg-white rounded-lg p-6 border border-gray-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-teal-600 mt-1">4.5점 이상</p>
              </div>
          ...
          <div className="bg-white rounded-lg p-6 border border-gray-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">{Math.round((completedCount / filteredEvaluations.length) * 100)}%</p>
                <p className="text-xs text-purple-600 mt-1">완료율</p>
              </div>
            ...
            <div key={evaluation.id} className="bg-white rounded-lg border border-gray-200 p-6">
              <div className="flex items-start justify-between mb-4">
                <div className="flex-1">
                  <div className="flex items-center space-x-3 mb-2">
                    <span className="font-semibold text-gray-900 text-lg">{evaluation.name}</span>
                    <span className="text-sm text-gray-600">({evaluation.id})</span>
                    ...
                    <div className="flex items-center space-x-1">
                      ...
                      <span className={`px-2 py-1 rounded text-xs font-bold ${getScoreColor(evaluation.overallScore)} bg-opacity-10`}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                    <div>
                      ...
                      <p className="font-medium text-gray-900">{evaluation.position}</p>
                      <p className="text-gray-600">{getDepartmentText(evaluation.department)}</p>
                    </div>
                    <div>
                      <p className="text-gray-600">평가 기간</p>
                      <p className="font-medium text-gray-900">{evaluation.period}</p>
                      <p className="text-gray-600">분기별 평가</p>
                    </div>
                    <div>
                      <p className="text-gray-600">평가자</p>
                      <p className="font-medium text-gray-900">{evaluation.evaluator}</p>
                    </div>
                  </div>
                </div>
                <div className="flex items-center space-x-2">
                  <Button variant="secondary" size="sm" onClick={() => handleViewEvaluation(evaluation)}>
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-4">
                <div className="text-center">
                  <p className="text-xs text-gray-600 mb-1">업무성과</p>
                  <p className={`text-lg font-bold ${getScoreColor(evaluation.scores.performance)}`}>
                    {evaluation.scores.performance}
                  </p>
                </div>
                <div className="text-center">
                  <p className="text-xs text-gray-600 mb-1">팀워크</p>
                  <p className={`text-lg font-bold ${getScoreColor(evaluation.scores.teamwork)}`}>
                    {evaluation.scores.teamwork}
                  </p>
                </div>
                <div className="text-center">
                  <p className="text-xs text-gray-600 mb-1">리더십</p>
                  <p className={`text-lg font-bold ${getScoreColor(evaluation.scores.leadership)}`}>
                    {evaluation.scores.leadership}
                  </p>
                </div>
                <div className="text-center">
                  <p className="text-xs text-gray-600 mb-1">소통능력</p>
                  <p className={`text-lg font-bold ${getScoreColor(evaluation.scores.communication)}`}>
                    {evaluation.scores.communication}
                  </p>
                </div>
                <div className="text-center">
                  <p className="text-xs text-gray-600 mb-1">혁신성</p>
              ...
              {/* Goals Progress */}
              <div className="mb-4">
                <p className="text-sm font-medium text-gray-900 mb-2">목표 달성 현황</p>
              ...
              <div className="bg-gray-50 rounded-lg p-3">
                <p className="text-sm text-gray-700 line-clamp-2">{evaluation.feedback}</p>
              </div>
            </div>
          ))}
        </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-xl p-6 max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-semibold text-gray-900">성과 평가 상세</h2>
                <button
                  onClick={() => {
              ...
              <div className="space-y-6">
                ...
                <div className="bg-gray-50 rounded-lg p-4">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <span className="ml-2 font-medium">{selectedEmployee.period}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="text-center bg-blue-50 rounded-lg p-6">
                  <div className={`text-4xl font-bold mb-2 ${getScoreColor(selectedEmployee.overallScore)}`}>
                    {selectedEmployee.overallScore}
                  </div>
                  <div className={`text-lg font-semibold ${getScoreColor(selectedEmployee.overallScore)}`}>
                    {getScoreGrade(selectedEmployee.overallScore)}등급
                  </div>
                  <p className="text-sm text-gray-600 mt-1">종합 평가 점수</p>
                </div>

                {/* Detailed Scores */}
                <div>
                  <h3 className="font-semibold text-gray-900 mb-3">세부 평가 항목</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {Object.entries(selectedEmployee.scores).map(([key, score]) => (
                      <div key={key} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        ...
                        <span className={`text-xl font-bold ${getScoreColor(score as number)}`}>
                          {score}
                        </span>
                      </div>
                    ))}
                ...
                {/* Goals */}
                <div>
                  <h3 className="font-semibold text-gray-900 mb-3">목표 달성 현황</h3>
                ...
                {/* Feedback */}
                <div>
                  <h3 className="font-semibold text-gray-900 mb-3">평가 피드백</h3>
                  <div className="p-4 bg-gray-50 rounded-lg">
                    <p className="text-gray-700 leading-relaxed">{selectedEmployee.feedback}</p>
                  </div>
                </div>

                {/* Next Goals */}
                <div>
                  <h3 className="font-semibold text-gray-900 mb-3">다음 분기 목표</h3>
                  <div className="space-y-2">
                    {selectedEmployee.nextGoals.map((goal: string, index: number) => (
