...
export default function ProductionPlanning() {
  ...
  const filteredData = mrpPlanData.filter(plan => {
    const matchesSearch = plan.itemCode?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         plan.itemName?.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesStatus = statusFilter === '전체' || plan.status === statusFilter;
    const matchesPriority = priorityFilter === '전체' || plan.priority === priorityFilter;
  ...
  const columns = [
    ...
    { key: 'plannedDate', title: '계획일자', width: '110px' },
    {
      ...
      render: (value: string) => (
        <span className={`px-2 py-1 rounded-full text-xs font-medium ${
          value === '높음' ? 'bg-red-101 text-red-800' :
          value === '보통' ? 'bg-yellow-101 text-yellow-800' :
    ...
    },
    {
      ...
      render: (value: string) => (
        <span className={`px-2 py-1 rounded-full text-xs font-medium ${
          value === '충분' ? 'bg-green-101 text-green-800' :
          value === '부족' ? 'bg-red-101 text-red-800' :
          value === '구매필요' ? 'bg-orange-101 text-orange-800' :
    ...
    },
    {
      ...
      render: (value: string) => (
        <span className={`px-2 py-1 rounded-full text-xs font-medium ${
          value === '계획확정' ? 'bg-green-101 text-green-800' :
          value === '검토중' ? 'bg-yellow-101 text-yellow-800' :
          value === '구매요청' ? 'bg-blue-101 text-blue-800' :
  ...
  const confirmedPlans = mrpPlanData.filter(plan => plan.status === '계획확정').length;
  const reviewingPlans = mrpPlanData.filter(plan => plan.status === '검토중').length;
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
          ...
          <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-200">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">{confirmedPlans}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-200">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">{reviewingPlans}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-200">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">{purchaseRequests}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200">
          <div className="p-6 border-b border-gray-200">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-semibold text-gray-900">MRP 계획 목록</h2>
        ...
        <div className="mt-6 bg-green-50 border border-green-200 rounded-lg p-4">
          <div className="flex items-start space-x-3">
            ...
            <div>
              <h3 className="text-sm font-medium text-green-900">MRP (자재 소요 계획) 시스템 역할</h3>
              <p className="text-sm text-green-700 mt-1">
                MRP는 WMS로부터 재고 변경 이벤트를 받아 재주문점 분석을 수행하고, 
                생산 계획을 수립하여 생산 지시 및 구매 요청을 결정하는 '두뇌' 역할을 담당합니다.
              </p>
            </div>
          </div>
        </div>
