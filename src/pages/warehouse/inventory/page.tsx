...
export default function WarehouseInventory() {
  ...
  const totalValue = inventory.reduce(
    (sum, i) => sum + i.current * i.unitPrice,
    0,
  );
      ...
      <div className="-1 p-6">
                ...
                >
                  <div className="mb-3 items-start justify-between">
                    <div className="-1">
                      <div className="mb-2 flex items-center space-x-3">
                        ...
                        <span className="rounded-full bg-gray-200 px-2 py-1 text-xs text-gray-700 dark:bg-gray-600 dark:text-gray-300">
                          {getCategoryText(item.category)}
                        </span>
                        <div className="flex items-center space-x-1">
                          <i className={getMovementIcon(item.movementType)}></i>
                          <span className="text-xs text-gray-500 dark:text-gray-400">
                            {item.movementType === "in" ? "입고" : "출고"} (
                            {item.lastMovement})
                          </span>
                        </div>
                      </div>
                  ...
                  <div className="mt-3">
                    <div className="mb-1 flex items-center justify-between text-sm text-gray-600 dark:text-gray-400">
                      <span>재고 수준</span>
                      <span>
                        {Math.round((item.current / item.maximum) * 100)}%
                      </span>
                    </div>
                    <div className="h-2 w-full rounded-full bg-gray-200 dark:bg-gray-600">
                      <div
                        ...
                        style={{
                          width: `${Math.min((item.current / item.maximum) * 100, 100)}%`,
                        }}
                      ></div>
                    </div>
                  </div>
        ...
        {showAddStockModal && (
          <div className="bg-opacity-50 fixed inset-0 z-50 flex items-center justify-center bg-black">
            <div className="mx-4 w-full max-w-md rounded-xl bg-white p-6 transition-colors duration-200 dark:bg-gray-800">
              ...
              <form onSubmit={handleSubmitAddStock} className="space-y-4">
                ...
                <div className="space-x-3 pt-4">
                  <Button type="submit" variant="default" className="-1">
                    입고 등록
                  </Button>
                  <Button
                    type="button"
          ...
          <div className="bg-opacity-50 fixed inset-0 z-50 flex items-center justify-center bg-black">
            <div className="mx-4 w-full max-w-md rounded-xl bg-white p-6 transition-colors duration-200 dark:bg-gray-800">
              ...
              <form onSubmit={handleSubmitEdit} className="space-y-4">
                <Input label="부품 코드" value={selectedItem.code} disabled />
                <Input label="부품명" defaultValue={selectedItem.name} />
                <Input
                  label="최소 재고"
                  type="number"
                ...
                />
                <Input label="보관 위치" defaultValue={selectedItem.location} />
                <Input label="공급업체" defaultValue={selectedItem.supplier} />
                <div className="space-x-3 pt-4">
                  <Button type="submit" variant="default" className="-1">
                    변경사항 저장
                  </Button>
        ...
        {showBarcodeScanner && (
          <div className="bg-opacity-50 fixed inset-0 z-50 flex items-center justify-center bg-black">
            <div className="mx-4 w-full max-w-sm rounded-xl bg-white p-6 transition-colors duration-200 dark:bg-gray-800">
              <div className="text-center">
                <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-blue-100 dark:bg-blue-900">
                  <i className="ri-barcode-line text-2xl text-blue-600 dark:text-blue-400"></i>
                </div>
